package com.happy_time.happy_time.ddd.shift_assignment.controller;
...
@RestController
@RequestMapping(path = "/api/shift_assignment")
public class ShiftAssignmentController {
    ...
    @PostMapping("/search")
    public Optional<ResponseObject> search(HttpServletRequest httpServletRequest, @RequestParam("page") Integer page, @RequestParam("size") Integer size, @RequestBody CommandShiftAssignment command) {
        try {
            ...
            Page<ShiftAssignment> assignments = shiftAssignmentApplication.search(command, page, size);
            List<ShiftAssignment> list = assignments.getContent();
            if (assignments.getTotalElements() > 0L) {
                Paginated<ShiftAssignment> total_configs = new Paginated<>(list, assignments.getTotalPages(), assignments.getSize(), assignments.getTotalElements());
                ResponseObject res = ResponseObject.builder().status(9999).message("success").payload(total_configs).build();
                return Optional.of(res);
            } else {
                Paginated<ShiftAssignment> total_configs = new Paginated<>(new ArrayList<>(), 0, 0, 0);
                ResponseObject res = ResponseObject.builder().status(9999).message("success").payload(total_configs).build();
                return Optional.of(res);
            }
        } catch (Exception e) {
    ...
    @PostMapping("/create")
    public Optional<ResponseObject> create(HttpServletRequest httpServletRequest, @RequestBody ShiftAssignment assignment) throws Exception {
        try {
            String tenant_id = tokenUtils.getFieldValueThroughToken(httpServletRequest, "tenant_id");
            ...
            ReferenceData ref = ReferenceData.builder()
                    ...
                    .build();
            assignment.setTenant_id(tenant_id);
    ...
    public Optional<ResponseObject> update(HttpServletRequest httpServletRequest, @RequestBody CommandShiftAssignment command,  @PathVariable ObjectId id) {
        try {
            String tenant_id = tokenUtils.getFieldValueThroughToken(httpServletRequest, "tenant_id");
            String agent_id = tokenUtils.getFieldValueThroughToken(httpServletRequest, "agent_id");
            String name = tokenUtils.getFieldValueThroughToken(httpServletRequest, "name");
            ...
            ReferenceData ref = ReferenceData.builder()
                    ...
                    .build();
            command.setLast_update_by(ref);
            ShiftAssignment edited = shiftAssignmentApplication.update(command, id.toHexString());
    ...
    public Optional<ResponseObject> delete(HttpServletRequest httpServletRequest, @PathVariable ObjectId id) {
        try {
            ...
            Boolean is_deleted = shiftAssignmentApplication.delete(id);
    ...
    public Optional<ResponseObject> getById(HttpServletRequest httpServletRequest, @PathVariable ObjectId id) {
        try {
            ...
            ShiftAssignment ShiftAssignment = shiftAssignmentApplication.getById(id);
            ResponseObject res = ResponseObject.builder().status(9999).message("success").payload(ShiftAssignment).build();
            return Optional.of(res);
        }
        catch (Exception e) {
            ResponseObject res = ResponseObject.builder().status(-9999).message("failed").payload(e.getMessage()).build();
